...
def recursively_create_hypertext(
    ...
    for piece in pieces:
        if isinstance(piece, list):
            result.append(recursively_insert_hypertext(piece, datastore, pointer_link_map))
        else:
            try:
...
def recursively_insert_hypertext(
    ...
    return datastore.insert(recursively_create_hypertext(pieces, datastore, pointer_link_map))
...
def insert_raw_hypertext(
    ...
    return recursively_insert_hypertext(parsed, datastore, pointer_link_map)
...
def create_raw_hypertext(
    ...
    parsed = hypertext.parse(content)
    return recursively_create_hypertext(parsed, datastore, pointer_link_map)
